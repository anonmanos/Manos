...
// ค่า action = add,insert,edit,update,delete
 ...
 <ul class="breadcrumb">
  ...
  <li>
    <a href="index.php?mod=admin&name=admin">ผู้ดูแลระบบ</a> <span class="divider">/</span>
  </li>
  <li class="active">
...
//ส่วนที่ 2
List($goto,$totalpage) = SelectPage($page,$each,"",USER,"id!='1' AND level='A'");
?>

<a href="index.php?mod=admin&name=admin&act=add" class="btn btn-success"><i class="icon-plus icon-white"></i> เพิ่มผู้ดูแลระบบใหม่</a>
...
//ส่วนที่ 3
ConnectDB();
$sql="SELECT id,username,CONCAT(pname,fname,' ',lname) As fullname,sex,email,level,status,login FROM ".USER." WHERE id!='1' AND level='A' LIMIT ".$goto.",".$each;
$query = QueryDB($sql);
$i = 1;
      ...
      <td>
		<div class="btn-group">
          ...
          <ul class="dropdown-menu">
            <li><a href="index.php?mod=admin&name=admin&act=edit&id=<?php echo $rs['id'];?>"><i class="icon-pencil"></i> แก้ไขข้อมูล</a></li>
		...
		<div id="delete_<?php echo $i;?>" class="modal hide fade" style="display: none;">
			...
			<div class="modal-body">
				<p>คุณต้องการลบข้อมูลผู้ดูแลระบบ  : <u><?php echo $rs['fullname'];?></u>  นี้หรือไม่ ?></p>
			</div>
			<div class="modal-footer">
				<a class="btn btn-primary" href="index.php?mod=admin&name=admin&act=delete&id=<?php echo $rs['id'];?>">ตกลง</a>
		...
		<div id="lock_<?php echo $i;?>" class="modal hide fade" style="display: none;">
			...
			<div class="modal-body">
				<p>คุณต้องการให้<?php echo $STATUS[$rs['status']]['v'];?>ข้อมูลผู้ดูแลระบบ  : <u><?php echo $rs['fullname'];?></u>  นี้หรือไม่ ?></p>
			</div>
			<div class="modal-footer">
				<a class="btn btn-primary" href="index.php?mod=admin&name=admin&act=lock&id=<?php echo $rs['id'];?>&status=<?php echo $STATUS[$rs['status']]['k'];?>">ตกลง</a>
				<a data-dismiss="modal" class="btn" href="#">ยกเลิก</a>
			</div>
		</div>
	  </td>
...
<?php
//ส่วนที่ 4
echo SplitPage($page,$totalpage,"index.php?mod=admin&name=admin");
...
?>
<form class="well" method="post" action="index.php?mod=admin&name=admin&act=insert" name="saciw" onSubmit="return formCheck()">
...
if($act==='insert'):
	...
	if($row>0){
		$ms = array(
			'message'=>' <strong>ข้อมูลซ้ำกันนะครับ<br />ปรับเปลี่ยนข้อมูลครับ</strong>',
			'class'=>'btn-warning',
			'yes'=>'',
			'no'=>'',
		);
		...
		if(InsertDB(USER,$sql)){
			$ms = array(
				'message'=>' <strong>บันทึกข้อมูลเรียบร้อยครับ<br />ต้องการทำรายการต่อหรือไม่ ?</strong>',
				'class'=>'btn-success',
				'yes'=>'index.php?mod=admin&name=admin&act=add',
				'no'=>'index.php?mod=admin&name=admin',
			);
			ShowProcess($ms);
		}else{
			$ms = array(
...
CloseDB();
?>
<form class="well" method="post" action="index.php?mod=admin&name=admin&act=update&id=<?php echo $rs['id'];?>" name="saciw" onSubmit="return formCheck()">
<h3>ข้อมูลใช้ในระบบ</h3>
  <label>ชื่อผู้ใช้</label>
...
if($act==='update'):
	...
	if(UpdateDB(USER,$sql,"id='".trim($_GET['id'])."'")){
		$ms = array(
				'message'=>' <strong>แก้ไขข้อมูลเรียบร้อยครับ<br />ต้องการแก้ไขข้อมูลนี้อีกครั้งหรือไม่ ?</strong>',
				'class'=>'btn-success',
				'yes'=>'index.php?mod=admin&name=admin&act=edit&id='.trim($_GET['id']),
				'no'=>'index.php?mod=admin&name=admin',
		);
		ShowProcess($ms);
	}else{
...
if($act==='delete'):
	...
	if(DeleteDB(USER,"id='".trim($_GET['id'])."'")){
		/*$ms = array(
				'message'=>' <strong>ลบข้อมูลเรียบร้อยครับ</strong>',
				'class'=>'btn-success',
				'yes'=>'index.php?mod=admin&name=admin',
				'no'=>'',
		);
		ShowProcess($ms);*/
		echo '<meta http-equiv="refresh" content="0;URL=index.php?mod=admin&name=admin">';
	}else{
		$ms = array(
...
function formCheck() {
	...
	if (obj.pass_cfm.value == false) {
		alert("กรุณายืนยันรหัสผ่านครับ");
		obj.pass_cfm.focus();
		return false;
	}
	if (obj.pass_new.value != obj.pass_cfm.value) {
		alert("รหัสผ่านยืนยันไม่ถูกต้องครับ");
		obj.pass_cfm.focus();
		return false;
	}
	if (obj.pname.value == false) {
		alert("กรุณากรอกคำนำหน้าชื่อครับ");
		obj.pname.focus();
		return false;
	}
	if (obj.fname.value == false) {
		alert("กรุณากรอกชื่อครับ");
		obj.fname.focus();
		return false;
	}
	...
	if(obj.email.value == false || isValidEmail(obj.email.value) == false)) {
		alert("กรุณากรอกอีเมล์ให้ถูกต้องครับ");
		obj.email.focus();
		return false;
	}
}
