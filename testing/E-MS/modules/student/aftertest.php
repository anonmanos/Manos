...
//ตรวจสอบว่าได้ทำแบบทดสอบก่อนเรียน ป้องกันการทำซ้ำๆ
ConnectDB();
$sql = "SELECT sc.aftertest FROM ".STADY." As st LEFT JOIN ".SCORE." As sc ON(st.id=sc.id) WHERE st.major_id='".$_GET['majorId']."' AND st.author_id='".$_SESSION['_id']."' AND sc.lsid='".$_GET['lessonId']."' LIMIT 1";
$query = QueryDB($sql);
$rs = FetchDB($query);
CloseDB();
if($rs['aftertest']!=''):
	$ms = array(
		'message'=>'<strong>ไม่สามารถทำแบบทดสอบก่อนเรียนได้<br/>เนื่องจากนักเรียนได้ทำแบบทดสอบก่อนเรียนไปแล้ว</strong>',
		'class'=>'btn-danger',
...
<form class="well" method="post" action="index.php?mod=student&name=aftertest&act=insert&majorId=<?php echo $_GET['majorId'];?>&lessonId=<?php echo $_GET['lessonId'];?>" name="saciw">
<?php
ConnectDB();
...
<label id="title">แบบทดสอบก่อนเรียน&nbsp;มีทั้งหมด&nbsp;<span id="number"><?php echo countAll(TEST,'lsid',"lsid='".$_GET['lessonId']."' AND code='A'",false);?></span>&nbsp;ข้อ</label>
<?php
ConnectDB();
$sql="SELECT id,title FROM ".TEST." WHERE lsid='".$_GET['lessonId']."' AND code='A' ORDER BY RAND()";
$query = QueryDB($sql);
$i = 1;
$radio = array();
...
if($act==='insert'):
	...
	if(InsertDB(SCORE,$sql)){
		$ms = array(
					'message'=>' <strong>บันทึกข้อมูลเรียบร้อย<br />คะแนนแบบทดสอบก่อนเรียน '.$rs['lesson'].' ได้ '.$ansCount.' คะแนน</strong>',
					'class'=>'btn-success',
					'yes'=>'index.php?mod=student&name=lesson&majorId='.$_GET['majorId'].'&lessonId='.$_GET['lessonId'],
					'no'=>'#',
		);
