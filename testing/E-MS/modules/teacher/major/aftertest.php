...
?>
 <ul class="breadcrumb">
  ...
  <li>
    <a href="index.php?mod=teacher&name=major/aftertest&id=<?php echo $_GET['id'];?>">แบบทดสอบก่อนเรียน</a> <span class="divider">/</span>
  </li>
  <li class="active">
...
<ul class="nav nav-tabs" id="tab">
    ...
    <li class="active"><a href="index.php?mod=teacher&name=major/aftertest&id=<?php echo $_GET['id'];?>">แบบทดสอบก่อนเรียน&nbsp;(<?php echo countAll(LESS.' As l INNER JOIN '.TEST.' As t ON(l.id=t.lsid)','l.mid',"l.mid='".$_GET['id']."' AND code='A'",false);?>)</a></li>     
...
//ส่วนที่ 2
List($goto,$totalpage) = SelectPage($page,$each,"t.id",LESS.' As l INNER JOIN '.TEST.' As t ON(l.id=t.lsid)',"l.mid='".$_GET['id']."' AND t.code='A'".$lesson_sql);
?>
<a href="index.php?mod=teacher&name=major/aftertest&act=add&id=<?php echo $_GET['id'];?>" class="btn btn-success"><i class="icon-plus icon-white"></i> เพิ่มข้อสอบใหม่</a>
...
//ส่วนที่ 3
ConnectDB();
$sql="SELECT t.id,t.title,t.answer,l.chapter FROM ".LESS." As l INNER JOIN ".TEST." As t ON(l.id=t.lsid) WHERE l.mid='".$_GET['id']."' AND t.code='A' ".$lesson_sql." ORDER BY t.lsid  LIMIT ".$goto.",".$each;
$query = QueryDB($sql);
$i = 1;
while($rs = FetchDB($query)):
  ?>
      ...
      <td>
		<div class="btn-group">
          ...
          <ul class="dropdown-menu">
            <li><a href="index.php?mod=teacher&name=major/aftertest&act=edit&testid=<?php echo $rs['id'];?>&id=<?php echo $_GET['id'];?>"><i class="icon-pencil"></i> แก้ไขข้อมูล</a></li>
		...
		<div id="delete_<?php echo $i;?>" class="modal hide fade" style="display: none;">
			...
			<div class="modal-footer">
				<a class="btn btn-primary" href="index.php?mod=teacher&name=major/aftertest&act=delete&testid=<?php echo $rs['id'];?>&id=<?php echo $_GET['id'];?>">ตกลง</a>
...
<script>
    $("select").change(function () {
		id = $(this).val();
		if(id!='')
			location.href='index.php?mod=teacher&name=major/aftertest&id=<?php echo $_GET['id']?>&lesson='+id;
		else
			location.href='index.php?mod=teacher&name=major/aftertest&id=<?php echo $_GET['id']?>';
    });
</script>

<?php
//ส่วนที่ 4
echo SplitPage($page,$totalpage,"index.php?mod=teacher&name=major/aftertest&id=".$_GET['id'].(empty($lesson)?'':'&lesson='.$lesson));
...
<form class="well" method="post" action="index.php?mod=teacher&name=major/aftertest&act=insert&id=<?php echo $_GET['id'];?>" name="saciw" onSubmit="return formCheck()">
  <label>บทเรียน</label>
  <select name="lsid">
...
if($act==='insert'):
	#print_r($_POST);exit();
	ConnectDB();
	$sql ="SELECT id FROM ".TEST." WHERE lsid='".$_POST['lsid']."' AND title='".$_POST['title']."' AND code='A'";
	$res = QueryDB($sql) ;
	$row = RowsDB($res) ;
	if($row>0){
		$ms = array(
		...
		if(InsertDB(TEST,$sql)){
			...
			$ms = array(
					...
					'class'=>'btn-success',
					'yes'=>'index.php?mod=teacher&name=major/aftertest&act=add&id='.$_GET['id'],
					'no'=>'index.php?mod=teacher&name=major/aftertest&id='.$_GET['id'],
			);
			ShowProcess($ms);			
		}else{
			$ms = array(
...
<script type="text/javascript" src="ckeditor/ckeditor.js"></script>
<form class="well" method="post" action="index.php?mod=teacher&name=major/aftertest&act=update&testid=<?php echo $_GET['testid'];?>&id=<?php echo $_GET['id'];?>" name="saciw" onSubmit="return formCheck()">
  <label>บทเรียน</label>
  <select name="lsid">
	<option value="">เลือกบทเรียน</option>
	<?php
...
if($act==='update'):
	...
	if(UpdateDB(TEST,$sql,"id='".$testid."'")){
		...
		$ms = array(
				...
				'class'=>'btn-success',
				'yes'=>'index.php?mod=teacher&name=major/aftertest&act=edit&testid='.$_GET['testid'].'&id='.$_GET['id'],
				'no'=>'index.php?mod=teacher&name=major/aftertest&id='.$_GET['id'],
		);
		ShowProcess($ms);
	}else{
...
if($act==='delete'):
	...
	if(DeleteDB(TEST,"id='".$_GET['testid']."'")){
		echo '<meta http-equiv="refresh" content="0;URL=index.php?mod=teacher&name=major/aftertest&id='.$_GET['id'].'">';
	}else{
		$ms = array(
