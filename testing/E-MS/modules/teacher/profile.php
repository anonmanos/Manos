 ...
 if($act==='data'):
	...
	if(UpdateDB(USER,$sql,"username='".$_SESSION['_username']."'")){
		...
		$ms = array(
				...
				'class'=>'btn-success',
				'yes'=>'index.php?mod=teacher&name=profile#data',
				'no'=>'index.php?mod=teacher',
		);
		ShowProcess($ms);
	}else{
		$ms = array(
 ...
 if($act==='pass'):
	...
	if(UpdateDB(USER,array('password'=>PassEnCode(trim($_POST['pass_new']))),"username='".$_SESSION['_username']."'")){
		$ms = array(
				'message'=>' <strong>แก้ไขข้อมูลเรียบร้อย<br />ต้องการแก้ไขข้อมูลนี้อีกครั้งหรือไม่ ?</strong>',
				'class'=>'btn-success',
				'yes'=>'index.php?mod=teacher&name=profile#pass',
				'no'=>'index.php?mod=teacher',
		);
		ShowProcess($ms);
	}else{
...
<div class="tab-content" id="myTabContent">
	...
	<div id="data" class="tab-pane fade active in">
		<form class="well" name="data"  method="post" action="index.php?mod=teacher&name=profile&act=data" onSubmit="return formCheckdata()">
...
function formCheckdata() {
	...
	<div id="pass" class="tab-pane fade">
		<form class="well" name="pass" method="post" action="index.php?mod=teacher&name=profile&act=pass" onSubmit="return formCheckpass()">
