...
// ค่า action = add,insert,edit,update,delete
 ...
 <ul class="breadcrumb">
  ...
  <li>
    <a href="index.php?mod=admin&name=teacher">ครูผู้สอน</a> <span class="divider">/</span>
  </li>
  <li class="active">
...
//ส่วนที่ 2
List($goto,$totalpage) = SelectPage($page,$each,"",USER,"level='T'");
?>

<a href="index.php?mod=admin&name=teacher&act=add" class="btn btn-success"><i class="icon-plus icon-white"></i> เพิ่มครูผู้สอนใหม่</a>
...
//ส่วนที่ 3
ConnectDB();
$sql="SELECT id,username,CONCAT(pname,fname,' ',lname) As fullname,sex,email,level,status,login FROM ".USER." WHERE level='T' LIMIT ".$goto.",".$each;
$query = QueryDB($sql);
$i = 1;
      ...
      <td>
		<div class="btn-group">
          ...
          <ul class="dropdown-menu">
            <?php /*<li><a href="index.php?mod=admin&name=teacher&act=edit&id=<?php echo $rs['id'];?>"><i class="icon-pencil"></i> แก้ไขข้อมูล</a></li>*/ ?>
		...
		<div id="delete_<?php echo $i;?>" class="modal hide fade" style="display: none;">
			...
			<div class="modal-body">
				<p>คุณต้องการลบข้อมูลครูผู้สอน  : <u><?php echo $rs['fullname'];?></u>  นี้หรือไม่ ?></p>
			</div>
			<div class="modal-footer">
				<a class="btn btn-primary" href="index.php?mod=admin&name=teacher&act=delete&id=<?php echo $rs['id'];?>">ตกลง</a>
		...
		<div id="lock_<?php echo $i;?>" class="modal hide fade" style="display: none;">
			...
			<div class="modal-body">
				<p>คุณต้องการให้<?php echo $STATUS[$rs['status']]['v'];?>ข้อมูลครูผู้สอน  : <u><?php echo $rs['fullname'];?></u>  นี้หรือไม่ ?></p>
			</div>
			<div class="modal-footer">
				<a class="btn btn-primary" href="index.php?mod=admin&name=teacher&act=lock&id=<?php echo $rs['id'];?>&status=<?php echo $STATUS[$rs['status']]['k'];?>">ตกลง</a>
				<a data-dismiss="modal" class="btn" href="#">ยกเลิก</a>
			</div>
		</div>
	  </td>
...
<?php
//ส่วนที่ 4
echo SplitPage($page,$totalpage,"index.php?mod=admin&name=teacher");
...
?>
<form class="well" method="post" action="index.php?mod=admin&name=teacher&act=insert" name="saciw" onSubmit="return formCheck()">
	...
	if($row>0){
		...
		if(InsertDB(USER,$sql)){
			$ms = array(
				...
				'class'=>'btn-success',
				'yes'=>'index.php?mod=admin&name=teacher&act=add',
				'no'=>'index.php?mod=admin&name=teacher',
			);
			ShowProcess($ms);
		}else{
			$ms = array(
...
ConnectDB();
$sql = "SELECT id,username,pname,fname,lname,sex,email FROM ".USER." WHERE id='".trim($_GET['id'])."' LIMIT 1";
$query = QueryDB($sql) ;
$rs = FetchDB($query);
CloseDB();
?>
<form class="well" method="post" action="index.php?mod=admin&name=teacher&act=update&id=<?php echo $rs['id'];?>" name="saciw" onSubmit="return formCheck()">
<h3>ข้อมูลใช้ในระบบ</h3>
  <label>ชื่อผู้ใช้</label>
...
if($act==='update'):
	...
	if(UpdateDB(USER,$sql,"id='".trim($_GET['id'])."'")){
		$ms = array(
				...
				'class'=>'btn-success',
				'yes'=>'index.php?mod=admin&name=teacher&act=edit&id='.trim($_GET['id']),
				'no'=>'index.php?mod=admin&name=teacher',
		);
		ShowProcess($ms);
	}else{
...
if($act==='delete'):
	...
	if(DeleteDB(USER,"id='".trim($_GET['id'])."'")){
		/*$ms = array(
				...
				'class'=>'btn-success',
				'yes'=>'index.php?mod=admin&name=teacher',
				'no'=>'',
		);
		ShowProcess($ms);*/
		echo '<meta http-equiv="refresh" content="0;URL=index.php?mod=admin&name=teacher">';
	}else{
		$ms = array(
